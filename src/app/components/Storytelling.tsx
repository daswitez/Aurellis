  ...
  return (
    ...
    >
      <div className="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-8 md:px-16 min-h-screen flex items-center justify-center">
        ...
        <div className="sc-s1 absolute inset-0 flex flex-col items-center justify-center text-center px-5 pointer-events-none pt-[90px] lg:pt-[110px] pb-6">
        ...
        <div className="sc-s2 absolute inset-0 flex flex-col items-center justify-center px-4 sm:px-6 pointer-events-none pt-[90px] lg:pt-[110px] pb-6">
          ...
          <div className="w-full flex flex-col md:grid md:grid-cols-3 gap-3 md:gap-5 max-w-5xl">
            ...
            <div className="sc-card sc-card-2 flex md:flex-col items-center md:items-start gap-4 md:gap-0 p-5 md:p-8 rounded-[18px] border border-border bg-card shadow-md text-left">
              ...
              <div className="flex-1">
                <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-0.5 md:mb-3">Respuesta real</p>
                <h3 className="text-2xl md:text-4xl font-serif font-semibold text-foreground leading-none">&lt; 2% <span className="font-mono text-sm md:text-base font-normal text-muted-foreground">en frío</span></h3>
                <p className="hidden md:block text-sm text-muted-foreground leading-relaxed border-t border-border/50 pt-4 mt-4">
            ...
            <div className="sc-card sc-card-3 flex md:flex-col items-center md:items-start gap-4 md:gap-0 p-5 md:p-8 rounded-[18px] border border-border bg-card shadow-md text-left">
              ...
              <div className="flex-1">
                <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-0.5 md:mb-3">Riesgo de reputación</p>
                <h3 className="text-2xl md:text-4xl font-serif font-semibold text-foreground leading-none">Daño <span className="font-mono text-sm md:text-base font-normal text-muted-foreground">de marca</span></h3>
                <p className="hidden md:block text-sm text-muted-foreground leading-relaxed border-t border-border/50 pt-4 mt-4">
        ...
        <div className="sc-s3 absolute inset-0 flex flex-col items-center justify-center text-center px-5 pointer-events-none pt-[90px] lg:pt-[110px] pb-6">
          <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-8 md:mb-12">
        ...
        <div className="sc-s4 absolute inset-0 flex flex-col items-center justify-center px-4 sm:px-6 pointer-events-none overflow-y-auto pt-[90px] lg:pt-[110px] pb-6">
          
          {/* Heading */}
          <div className="sc-s4-heading text-center mb-6 lg:mb-8 2xl:mb-14 pt-2 md:pt-0 mt-8 md:mt-0 relative">
            {/* Ambient Tech Glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] md:w-[600px] h-[100px] md:h-[200px] bg-[#B5A47D]/5 blur-[80px] md:blur-[120px] rounded-full sm:pointer-events-none" />
            
            <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#B5A47D]/50 block mb-2 2xl:mb-6">
              La respuesta de Aurellis
            </span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-4xl 2xl:text-6xl font-serif text-[#F6F3EE] leading-[1.1] tracking-tight max-w-3xl mx-auto">
              Tu pipeline comercial, operando.<br />
              <span className="text-[#F6F3EE]/40 font-light italic text-lg sm:text-lg md:text-xl lg:text-2xl 2xl:text-3xl mt-1 2xl:mt-2 block">
          ...
          <div className="w-full max-w-5xl flex flex-col lg:grid lg:grid-cols-5 gap-3 lg:gap-4 2xl:gap-6 items-start">

            {/* Pipeline Rows */}
            <div className="lg:col-span-3 space-y-2 2xl:space-y-3 w-full">
              <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#F6F3EE]/30 mb-2 2xl:mb-5">Pipeline activo en vivo</p>
              ...
              {[
                ...
                <div
                  key={i}
                  className="sc-pipeline-row flex items-center gap-2 lg:gap-3 2xl:gap-3 p-2 lg:p-2.5 2xl:p-4 rounded-[12px] bg-[#161613]/50 backdrop-blur-md border border-[#F6F3EE]/10 hover:border-[#B5A47D]/30 hover:bg-[#161613]/80 transition-all duration-300"
                >
                  <div className={`w-8 h-8 lg:w-8 lg:h-8 2xl:w-9 2xl:h-9 rounded-full flex items-center justify-center shrink-0 ${row.active ? "bg-[#B5A47D]/20 shadow-[0_0_15px_rgba(181,164,125,0.2)]" : "bg-[#F6F3EE]/4"}`}>
                    <div className={`w-2 h-2 rounded-full ${row.active ? "bg-[#B5A47D] animate-pulse" : "bg-[#F6F3EE]/15"}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs md:text-sm font-medium truncate ${row.active ? "text-[#F6F3EE]/90" : "text-[#F6F3EE]/40"}`}>
                      {row.persona}
                    </p>
                    <p className={`font-mono text-[10px] md:text-xs mt-0.5 truncate ${row.active ? "text-[#F6F3EE]/50" : "text-[#F6F3EE]/20"}`}>
                      {row.firm}
                    </p>
                  </div>
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className={`font-mono text-[9px] md:text-[10px] px-2 md:px-3 py-0.5 md:py-1 rounded-full border whitespace-nowrap ${row.active ? "text-[#B5A47D] bg-[#B5A47D]/10 border-[#B5A47D]/30" : "text-[#F6F3EE]/30 bg-[#F6F3EE]/4 border-[#F6F3EE]/10"}`}>
                      {row.badge}
                    </span>
                    <span className="hidden sm:block font-mono text-[9px] text-[#F6F3EE]/20">{row.time}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="lg:col-span-2 w-full">
              <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#F6F3EE]/30 mb-2 2xl:mb-5">Resultados del sistema</p>

              {/* Mobile: 2-col compact grid. Desktop: stacked */}
              <div className="grid grid-cols-2 lg:grid-cols-1 gap-2 lg:gap-3 2xl:gap-4">
                {[
                  ...
                  <div key={i} className="sc-metric flex items-center gap-2 lg:gap-3 2xl:gap-3 p-2 lg:p-2.5 2xl:p-4 rounded-[12px] bg-[#161613]/50 backdrop-blur-md border border-[#F6F3EE]/10 hover:border-[#B5A47D]/30 transition-all duration-300">
                    <div className="w-8 h-8 lg:w-8 lg:h-8 2xl:w-9 2xl:h-9 rounded-xl flex items-center justify-center shrink-0 bg-[#B5A47D]/15 shadow-[inset_0_0_10px_rgba(181,164,125,0.1)]">
                      <Icon className="w-3.5 h-3.5 2xl:w-4 2xl:h-4 text-[#B5A47D]" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-mono text-[9px] md:text-[10px] text-[#F6F3EE]/30 uppercase tracking-widest leading-tight">{label}</p>
                      <p className="font-mono text-base lg:text-lg 2xl:text-xl font-medium mt-0.5 text-[#B5A47D]">{value}</p>
                    </div>
                  </div>
                ))}
              </div>

              <div className="sc-footer-line mt-3 2xl:mt-4 flex items-center gap-2 pt-2 2xl:pt-3 border-t border-[#F6F3EE]/8">
                <div className="w-1.5 h-1.5 rounded-full bg-[#B5A47D] animate-pulse shrink-0 shadow-[0_0_8px_rgba(181,164,125,0.8)]" />
                <p className="font-mono text-[10px] text-[#F6F3EE]/40">Sistema activo · sin intervención manual constante</p>
              </div>
            </div>
